...
public class SparkOrcReader implements Iterator<UnsafeRow>, Closeable {
  ...
  public UnsafeRow next() {
    ...
    // Resetting the writer leads to incorrect behavior, because the holder has
    // already skipped based the fixed space. *ugh*
    holder.reset();
  ...
  private static class TimestampConverter implements Converter {
    ...
    public void convert(UnsafeRowWriter writer, int column, ColumnVector vector,
      ...
      if (!vector.noNulls && vector.isNull[row]) {
        ...
        writer.write(column, ((TimestampColumnVector) vector).time[row]);
      }
    }
    ...
    public void convert(UnsafeArrayWriter writer, int element,
      ...
      if (!vector.noNulls && vector.isNull[row]) {
        ...
        writer.write(element, ((TimestampColumnVector) vector).time[row]);
      }
    }
  }
  ...
  private static class Decimal18Converter implements Converter {
    ...
    public void convert(UnsafeRowWriter writer, int column, ColumnVector vector,
      ...
      if (!vector.noNulls && vector.isNull[row]) {
        ...
        writer.write(column,
            new Decimal().set(DecimalHack.unscaledLong(v), precision, scale),
            precision, scale);
      }
    }
